...
describe('unblock', () => {
	...
	it('resolves on 204', async () => {
		...
		await expect(unblock(42)).resolves.toBeUndefined()
		expect(api.delete).toHaveBeenCalledWith('/users/me/blocked/42')
	})

	it('throws BadFollowOperation on 400', async () => {
		api.delete.mockResolvedValue({ status: 400 })
		await expect(unblock(42)).rejects.toBe(BadFollowOperation)
	})
