...
describe('unfollow', () => {
	...
	it('resolves on 204', async () => {
		...
		await expect(unfollow(42)).resolves.toBeUndefined()
		expect(api.delete).toHaveBeenCalledWith('/users/me/following/42')
	})

	it('throws BadFollowOperation on 400', async () => {
		api.delete.mockResolvedValue({ status: 400 })
		await expect(unfollow(42)).rejects.toBe(BadFollowOperation)
	})
