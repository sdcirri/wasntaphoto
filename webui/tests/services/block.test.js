...
describe('block', () => {
	...
	it('resolves on 200', async () => {
		...
		await expect(block(42)).resolves.toBeUndefined()
		expect(api.post).toHaveBeenCalledWith('/users/me/blocked/42')
	})

	it('throws BadFollowOperation on 400', async () => {
		api.post.mockResolvedValue({ status: 400 })
		await expect(block(42)).rejects.toBe(BadFollowOperation)
	})
